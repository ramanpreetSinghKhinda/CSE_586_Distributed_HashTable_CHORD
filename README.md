...
>  4. The content provider do **not need to handle concurrent node joins** and its assumed that a node join will only happen once the system completely processes the previous join.
>  5. The content provider do **not need to handle insert/query requests while a node is joining** and Its assumed  that insert/query requests will be issued only with a stable system.
>  6. There are always **5 nodes** in the system and it **not need to handle node leaves/failures**
>  7. We have fixed the ports & sockets: -
	a) Our app opens one server socket that listens on **Port 10000**.
	b) We use [**run_avd.py**](https://github.com/ramanpreet1990/CSE_586_Simplified_Amazon_Dynamo/blob/master/Scripts/run_avd.py) and [**set_redir.py**](https://github.com/ramanpreet1990/CSE_586_Simplified_Amazon_Dynamo/blob/master/Scripts/set_redir.py) scripts to set up the testing environment.
